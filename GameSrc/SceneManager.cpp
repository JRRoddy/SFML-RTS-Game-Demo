...
SceneManager::SceneManager(sf::RenderWindow* window, unsigned int windowWidth, unsigned int windowHeight)
{
	...
	m_player = std::make_unique<Player>(Player( sf::Vector2f(window->getSize()) / 2.0f, m_spriteGenerator.get(), m_inputManager.get())); 
...
{
	m_levelGenerator->getCurrentAreaGrid()->draw(m_window);
