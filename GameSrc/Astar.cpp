...
{ 
	...
	std::vector<node*> path = {}; // path to be formed
	...
	while (openSet.size() > 0) {
		...
		for (gridTile * neighbourTile : m_levelGrid->getNeighbours(currentNode->indexX, currentNode->indexY)) 
		{
			...
			// it has already been evaled so skip it 
			if (containsNode(closedSet, *potentialNode)) {
				//std::cout << "skipping node as it is in closed set indicies x: "<< potentialNode->indexX<<" y:"<<potentialNode->indexY << std::endl;
				continue;
			}
